...
import tiktoken  # Package for tokenization
from services.openai_file import get_embeddings  # Custom service to get embeddings

# Global variables
tokenizer = tiktoken.get_encoding(
    "cl100k_base"
)  # Initialize the tokenizer using the "cl100k_base" encoding scheme
...
def get_text_chunks(text: str, chunk_token_size: Optional[int]) -> List[str]:
    ...
    """
    # Ensure the text is a string and return an empty list if it's empty or whitespace
    ...
    while tokens and num_chunks < MAX_NUM_CHUNKS:
        ...
        if not isinstance(chunk_text, str) or not chunk_text or chunk_text.isspace():
...
def create_document_chunks(
    ...
    if not isinstance(doc.text, str) or not doc.text or doc.text.isspace():
...
def get_entire_text(text: str) -> str:
    """
    Return the entire text without splitting it into chunks.
    ...
    """
    # Ensure the text is a string and return an empty string if it's not or if it's empty or whitespace
    return text if isinstance(text, str) and text and not text.isspace() else ""
